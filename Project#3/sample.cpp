...
const char *WINDOWTITLE = { "Project #3 - I-Shen Liao" };
...
void MjbSphere(float radius, int slices, int stacks)
{
	...
	for (int ilat = 0; ilat < NumLats; ilat++)
	{
		...
		for (int ilng = 0; ilng < NumLngs; ilng++)
		{
			...
			if (DistortOn)
			{
				p->s = (lng + M_PI) / (2.*M_PI) + 0.1 * cos(2.*(lat + M_PI/2. + Time));
				p->t = (lat + M_PI / 2.) / M_PI + 0.1 * cos(2.*(lat + M_PI/2. + Time));
			}
			else
			{
				p->s = (lng + M_PI) / (2.*M_PI);
...
InitGraphics( )
{
	...
	texture = BmpToTexture("/Users/eason/Documents/OSU/2017_Fall_Term/CS550/Project_3/Project#3/worldtex.bmp", &texWidth, &texHeight);
	if (texture == NULL || texWidth != 1024 || texHeight != 512)
	{
        printf("Fail to read the bmp file!");
		exit(1);
	}
