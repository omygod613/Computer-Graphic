...
const char *WINDOWTITLE = { "Project#4 - I-Shen Liao" };
...
Display( )
{
    ...
    //Sun
    SetPointLight(GL_LIGHT0, 0., 0., 0., 1., 0.7, 0.8);
    glPushMatrix();
	...
	//Moon
	SetSpotLight(GL_LIGHT1, 12. * cos(2 * M_PI * Time), 0., 12. * sin(2 * M_PI * Time), -cos(2 * M_PI * Time), 0., -sin(2 * M_PI * Time), 1., 1., 0.);
	glPushMatrix();
	glDisable(GL_LIGHTING);
	glColor3f(1., 1., 0.);
	glTranslatef(12. * cos(2 * M_PI * Time), 0., 12. * sin(2 * M_PI * Time));
    ...
    //WhiteLight
    SetPointLight(GL_LIGHT2, 0., 8., 0., 1., 1., 1);
    glPushMatrix();
    glDisable(GL_LIGHTING);
    glColor3f(1., 1., 1);
    glTranslatef(0., 12., 0.);
	...
	glPushMatrix();
	glShadeModel(GL_SMOOTH);
	glTranslatef(8 * cos(2 * M_PI * Time), 0., 8 * sin(2 * M_PI * Time));
	glRotatef(360.*Time, 0., 1., 0.);
	SetMaterial(0.5, 0.5, 0.5, 0);
...
InitGraphics( )
{
	...
	Texture = BmpToTexture("/Users/eason/Documents/OSU/2017_Fall_Term/CS550/Project_4/Project#4/worldtex.bmp", &Width, &Height);
